...
import gordon_help_live_chat from '../images/gordon_help_flows/live_chat.png'

import case_welcome from '../images/case_flows/welcome_message.png'
import case_in_line from '../images/case_flows/in_line.png'
...
- <b>Dynamic Lighting:</b> Implements a realistic lighting system where the brightness of each terrain feature is calculated based on the angle between its surface normal and a fixed light direction. This simulates day-night cycles and provides realistic shading, enhancing the visual depth and immersion of the planet.`,
  ...
  'chai': {
    ...
    - Customer Truck ETAs
    - Purchase Order Details & Tracking
    - Weekly KPI Tracking
  ...
  'gordonHelp': {
    ...
    'case': {
    title: 'C.A.S.E',
    overview: `C.A.S.E is an internal agent focused on answering warehouse onboarding questions using an internal teams documentation as a knowledge base with accessibility through Google Chat.
    
    <b>Key Features:</b>
    - In-line documentation system
    - Generative responses utilizing a knowledge base
    `,
    technologies: [dialogflowCXLogo, gemini],
    features: [{ imgSrc: case_welcome, title: 'Home Screen', description: 'The welcoming interface with navigation options.' }],
    welcome_image: case_welcome,
  },
  'sage': {
    title: 'Sage',
  ...
  return (
    <div className="project-detail py-12 bg-gray-900 text-gray-200 min-h-screen">
      <div className="container mx-auto px-4">
        ...
        {project.welcome_image && id !== 'chai' && id !== 'gordonHelp' && (
          <div>
            <img
        ...
        {project.welcome_image && id === 'gordonHelp' && (
          <div>
            <img
              src={project.welcome_image}
              alt="Tech Icon"
              className="w-100 h-80 m-2 border-2 border-gray-700 rounded"
            />
          </div>
        )}
        ...
        {/* Chatbots Structure */}
        {(id === 'expo' || id === 'tess' || id === 'chai' || id === 'sage' ||id === 'gordonHelp') && (
          <h3 className="text-3xl mt-8 mb-4 text-bone font-bold">Flows</h3>
        )}
        {(id === 'case') && (
          <h3 className="text-3xl mt-8 mb-4 text-bone font-bold">In Line Citations</h3>
        )}
        {id === 'expo' && (
          <>
        ...
        {id === 'gordonHelp' && (
          <>
            <h3 className="text-2xl mt-8 mb-4 text-bone">Live Agent</h3>
            <p>Allows employees to connect to a live agent via google chat, switches routing of user messages to our InContact platform to allow them to converse with IT Specialsits seamlessly.</p>
            <br />
            <div className="flex flex-wrap items-start"> {/* Changed from flex-row */}
              <img
                src={gordon_help_live_chat}
                alt="Gordon Help Live Chat"
                className="w-100 h-100 m-2 border-2 border-gray-700 rounded"
              />
            </div>
          </>
        )}
        {id === 'case' && (
          <>
            <p>This in-line citation system was inspired by Notebook LM and was a recommendation requirement specified by the internal team that this chatbot was a collaboration with. In-line citations give users better context to whioh portions of an agent's responses come from which document. Allowing them to easily identify and review source doucmentation where needed./</p>
            <br />
            <div className="flex flex-wrap items-start"> {/* Changed from flex-row */}
              <img
                src={case_in_line}
                alt="In Line Citations"
                className="w-100 h-100 m-2 border-2 border-gray-700 rounded"
              />
            </div>
          </>
